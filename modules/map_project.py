        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    ...
                    level = '.' * node.level
                    from_part = f"from {level}{module}" if module else f"from {'.' * node.level}"
                    if node.names:
                        names = ', '.join(alias.name for alias in node.names)
                        imports.append(f"{from_part} import {names}")
                    else:
                        imports.append(f"{from_part} import *")
        except SyntaxError:
            # Fall back to regex if AST fails
            pass
    # If not Python or AST failed, use regex for JS/TS and fallback for others
    if not imports:
        lines = content.split('\n')
        for line in lines:
            stripped = line.strip()
            # Python-style imports (already handled by AST, but keep as fallback)
            ...
            js_import_patterns = [
                ...
                r'^import\s+[\'*\s\w]+\s+from\s+[\'"]([^\'"]+)[\'"]',  # import * as foo from 'mod'
                r'^import\s+[\'"]([^\'"]+)[\'"]',  # import 'mod'
                r'^\s*(?:const|let|var)\s+[^=]*=\s*require\s*\(\s*[\'"]([^\'"]+)[\'"]\s*\)',
                # TypeScript export/import types
...
def get_exports(file_path):
    ...
    if file_path.endswith('.py'):
        ...
        # For JS/TS and others, use regex
        lines = content.split('\n')
        for line in lines:
            stripped = line.strip()
            ...
            elif re.match(r'^(?:const|let|var)\s+\w+\s*=\s*(?:async\s+)?\(.*\)\s*=>', stripped):
            ...
            # Class declarations: class Foo {}
                ...
                if match:
                    exports.append(match.group(1))
            # Export statements: export function foo {}, export class Bar {}, export const baz = ...
            elif re.match(r'^export\s+(?:async\s+)?function\s+\w+', stripped):
                match = re.search(r'export\s+(?:async\s+)?function\s+(\w+)', stripped)
                if match:
                    exports.append(match.group(1))
            elif re.match(r'^export\s+class\s+\w+', stripped):
                match = re.search(r'export\s+class\s+(\w+)', stripped)
                if match:
                    exports.append(match.group(1))
            elif re.match(r'^export\s+(?:const|let|var)\s+\w+', stripped):
                match = re.search(r'^export\s+(?:const|let|var)\s+(\w+)', stripped)
                if match:
                    exports.append(match.group(1))
